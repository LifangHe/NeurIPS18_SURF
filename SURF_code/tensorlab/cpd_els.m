...
if ~isfield(state,'fval') || ...
   (options.Scale && (~isfield(state,'K') || ~isfield(state,'K1')))
    if ~isfield(state,'fval')
        if isstructured
            state.fval = 0.5*state.T2 - real(inprod(T,U)) + 0.5*frob(U,'squared');
        else 
            D = cpdres(T,U);
            state.fval = 0.5*(D(:)'*D(:));
        end
...
for d = 1:N
    for i = find(deg.' == d)
        if isstructured
            tmp = mtkrprod(U,Uall(I(i,1:end)+2*(0:N-1)),0) - ...
                  mtkrprod(T,Uall(I(i,1:end)+2*(0:N-1)),0);
        else 
            tmp = mtkrprod(T,Uall(I(i,1:end)+2*(0:N-1)),0);
        end 
        tmp = conj(sum(tmp));
        p(d+1) = p(d+1)+tmp;
    end
