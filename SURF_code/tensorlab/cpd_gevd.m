function [U,output] = cpd_gevd(T,R,varargin)
...
%
% Version History:
% - 2016/02/28   NV      Added random slices option

% Incomplete/sparse tensors not supported yet.
T = fmt(T,true);
if isstruct(T)
    error('cpd_gevd:notyetsupported','This method does not yet support incomplete/sparse tensors.');
end
...
p = inputParser;
p.addOptional('Backup', @cpd_rnd);
p.addOptional('IsReal', 'auto');
p.addOptional('Slices', 'mlsvd');
p.KeepUnmatched = true;
p.parse(varargin{:});
options = p.Results;
if strcmpi(options.IsReal,'auto')
...
if R > 1
    ...
    U = cellfun(@(u,n) bsxfun(@rdivide, u, n), U, nrm, 'UniformOutput',0);
    nrm = nthroot(prod(cat(1, nrm{:}),1), length(U));
    U = cellfun(@(u) bsxfun(@times, u, nrm), U, 'UniformOutput',0); 
else
